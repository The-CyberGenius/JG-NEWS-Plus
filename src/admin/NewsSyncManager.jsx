...
// Keyword maps for website category auto-detection (Hindi + English)
...
const detectCategory = (title, content) => {
    ...
    // Priority order: crime > sports > entertainment > education > rural > default
    for (const cat of ['अपराध', 'खेल', 'मनोरंजन', 'शिक्षा', 'ग्रामीण']) {
...
export default function NewsSyncManager() {
    ...
    const handlePostAll = async () => {
        ...
        for (let i = 0; i < fetchedNews.length; i++) {
            ...
            try {
                ...
                const assignedCat = detectCategory(item.title, item.fullContent);
    ...
    const openPreview = (item) => {
        ...
        setPreviewItem({
            ...
            assignedCat: detectCategory(item.title, item.fullContent),
        });
    };
    ...
    const editAndPost = (item) => {
        ...
        const prefill = {
            ...
            excerpt: makeExcerpt(item.fullContent, 180),
            content: `${cleanedContent}<p><br/>Source: <a href="${item.link}" target="_blank">${item.source}</a></p>`,
            category: detectCategory(item.title, item.fullContent),
            location: isRajasthan ? detectLocation(item.title, item.fullContent) : 'अन्य',
            image: heroImg,
            videoUrl: '',
            author: 'AI Sync',
